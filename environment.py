...
class GenericWorld:
    ...
    def setup_logging(self):
        global _GAME_LOG_INITIALIZED
        self.logger = logging.getLogger('BombeRLeWorld')
        # Avoid accumulating multiple handlers across worlds
        try:
            self.logger.handlers.clear()
        except Exception:
            self.logger.handlers = []
        self.logger.setLevel(s.LOG_GAME)
        mode = "w" if not _GAME_LOG_INITIALIZED else "a"
        handler = logging.FileHandler(f'{self.args.log_dir}/game.log', mode=mode, encoding="utf-8", delay=True)
        handler.setLevel(logging.DEBUG)
        formatter = logging.Formatter('%(asctime)s [%(name)s] %(levelname)s: %(message)s')
        handler.setFormatter(formatter)
        self.logger.addHandler(handler)
        self.logger.propagate = False
        _GAME_LOG_INITIALIZED = True
                ...
                for a in self.active_agents:
                    if (not a.dead) and (a.x, a.y) in explosion.blast_coords:
                        ...
                        if a is explosion.owner:
                            ...
                            # Team-aware kill handling: penalize friendly fire among training agents
                            if getattr(explosion.owner, 'train', False) and getattr(a, 'train', False):
                                self.logger.info(f'Agent <{a.name}> blown up by teammate <{explosion.owner.name}> (friendly fire)')
                                # No positive kill score for friendly fire
                                explosion.owner.add_event(e.KILLED_TEAMMATE)
                            else:
...
class BombeRLeWorld(GenericWorld):
    ...
    def build_arena(self):
        ...
        # Shuffle agent order each round to randomize provided indices/roles
        active_agents = []
        perm_indices = self.rng.permutation(len(self.agents))
        agents_perm = [self.agents[i] for i in perm_indices]
        for agent, start_position in zip(agents_perm, self.rng.permutation(start_positions)):
    ...
    def poll_and_run_agents(self):
        ...
        # Debug: all actions for this step have been collected and applied
        try:
            latest_actions = {a.name: self.replay['actions'][a.name][-1] for a in self.active_agents}
            self.logger.debug(f'Collected all actions for step {self.step}: {latest_actions}')
        except Exception:
            pass
