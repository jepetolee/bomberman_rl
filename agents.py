...
class AgentRunner:
    ...
    def __init__(self, train, agent_name, code_name, result_queue):
        ...
        # Disable per-agent file logging; avoid creating agent-specific .log files
        try:
            self.wlogger.handlers.clear()
        except Exception:
            self.wlogger.handlers = []
        try:
            self.fake_self.logger.handlers.clear()
        except Exception:
            self.fake_self.logger.handlers = []
        self.wlogger.propagate = False
        self.fake_self.logger.propagate = False
        null_handler = logging.NullHandler()
        self.wlogger.addHandler(null_handler)
        self.fake_self.logger.addHandler(null_handler)
